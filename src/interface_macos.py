...
download_img = ImageTk.PhotoImage(
    Image.open(os.path.join(base_path, "icones/outil_telecharger.png")).resize(size_tool_button))
...
upload_img = ImageTk.PhotoImage(
    Image.open(os.path.join(base_path, "icones/outil_enregistrer.png")).resize(size_tool_button))
...
def push_open():
    ...
    file_path = filedialog.askopenfilename(title="Ouvrir un fichier",
                                           filetypes=[("Fichiers JSON,", ".JSON"), ("Fichiers TXT,", ".txt")])
    try:
        arbre = Tree()
...
save_img = ImageTk.PhotoImage(
    Image.open(os.path.join(base_path, "icones/outil_sauvegarder.png")).resize(size_tool_button))
...
zoom_moins_img = ImageTk.PhotoImage(
    Image.open(os.path.join(base_path, "icones/zoom_moins.png")).resize(size_tool_button))
...
def actualiser_dessin():
    ...
    l_NodeRectangle = [NodeRectangle(arbre.starting_node,
                                     [len(arbre.starting_node.coord) * 300, dic_y[arbre.starting_node.coord] * 100])]
    l_link = []  #arretes
    ...
    if not first_diaplay:
        ...
        canva.move(tk.ALL, -canva.coords(l_NodeRectangle[0].id_rectangle)[0] + xy_offset[0],
...
class NodeRectangle():
    ...
    def __init__(self, node=None, pos=[50, 50]):
        self.node = node
        self.pos = pos
        self.id_rectangle = canva.create_rectangle(self.pos[0], self.pos[1], (self.pos[0] + NodeRectangle.size[0]),
                                                   (self.pos[1] + NodeRectangle.size[1]))
        canva.itemconfig(self.id_rectangle, width=3)  #epaisseur du trait
        self.id_text = canva.create_text(self.pos[0] + NodeRectangle.size[0] / 2,
                                         self.pos[1] + NodeRectangle.size[1] / 2, text=node.name)
        ...
        self.char_size = len(node.name) / abs(
            canva.bbox(self.id_text)[0] - canva.bbox(self.id_text)[2])  #tailile mmoyenne d'un caractere
    ...
    def zoom(self, old_r_px):
        ...
        canva.coords(self.id_rectangle, x1 * r_px / old_r_px, y1 * r_px / old_r_px, x2 * r_px / old_r_px,
                     y2 * r_px / old_r_px)
...
class Link():
    def __init__(self, node_rectangle1, node_rectangle2):
        ...
        self.id_link = canva.create_line(node_rectangle1.pos[0] + NodeRectangle.size[0],
                                         node_rectangle1.pos[1] + NodeRectangle.size[1] // 2, node_rectangle2.pos[0],
                                         node_rectangle2.pos[1] + NodeRectangle.size[1] // 2)
        canva.itemconfig(self.id_link, width=3)  #epaisseur du trait

    def zoom(self, old_r_px):
        x1, y1, x2, y2 = canva.coords(self.id_link)
        canva.coords(self.id_link, x1 * r_px / old_r_px, y1 * r_px / old_r_px, x2 * r_px / old_r_px,
                     y2 * r_px / old_r_px)
...
def resize_window(event):  #Fonction appelée à chaque fois que la fenêtre est redimensionnée
    tools_frame.config(width=window.winfo_width() - 2 * e, height=bouton_download.winfo_height() + 2 * e)
    draw_frame.config(height=window.winfo_height() - 4 * e - tools_frame.winfo_height() - info_frame.winfo_height(),
                      width=window.winfo_width() - 2 * e)
    canva.config(
        height=window.winfo_height() - 4 * e - tools_frame.winfo_height() - 2 * bordure - info_frame.winfo_height(),
        width=window.winfo_width() - 2 * e - 2 * bordure)
    info_frame.config(height=2 * e + max(info_label.winfo_height(), find_close_button.winfo_height()),
                      width=window.winfo_width() - 2 * e)
